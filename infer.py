...
# MODIFIED: Import scipy for audio saving
...
def generate_audio(model_dict, cfg, csv_path, output_dir, guidance_scale=4.5, num_inference_steps=50, neg_prompt=None):
    ...
    for index, row in test_df.iterrows():
        ...
        if not os.path.exists(output_audio_path) or not os.path.exists(output_video_path):
            ...
            # MODIFIED: Use scipy to save audio instead of torchaudio.save
            audio_np = audio.cpu().numpy()
            # If shape is (channels, samples) and channels <= samples, assume (channels, samples) and transpose
            if audio_np.ndim == 2 and audio_np.shape[0] <= audio_np.shape[1]:
                audio_np = audio_np.T  # -> (samples, channels)
            # Convert float in [-1,1] to int16 if necessary
            if audio_np.dtype in (np.float32, np.float64):
                # Assume range [-1, 1]; adjust if your model outputs different range
                audio_int16 = (audio_np * 32767).clip(-32768, 32767).astype(np.int16)
            else:
                audio_int16 = audio_np
            scipy.io.wavfile.write(output_audio_path, sample_rate, audio_int16)
